...
assert(manifest.host_permissions === undefined, "host permissions are forbidden");
assert(
...
assert(manifest.action?.default_icon === "logo.png", "action logo path is wrong");
assert(
...
assert(!/<script[^>]+src=["']https?:/i.test(popup), "remote scripts are forbidden");
assert(!/<link[^>]+href=["']https?:/i.test(popup), "remote styles are forbidden");
...
console.log(`Validated ${allFiles.length} allowlisted extension files in dist/.`);
